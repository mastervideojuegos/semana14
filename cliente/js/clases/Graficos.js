//-------------Escenarios----------------//
var escenarios = [];

    //Escenario Negro
    var fondo_1     = new Image();
        fondo_1.src = "img/escenario_1.jpg"
    escenarios.push(fondo_1);

//-------------Bolas--------------------//
var bolas = [];

    //Bola Blanca
    var bola_1      = new Image();
        bola_1.src  = "img/bola_1.png";
    bolas.push(bola_1);

//-----------Jugadores------------------//
var jugadores = [];

    //Pad Amarillo
    var pad_1       = new Image();
        pad_1.src   = "img/pad_1.jpg";
    jugadores.push(pad_1);

    //Pad Azul
    var pad_2       = new Image();
        pad_2.src   = "img/pad_2.jpg";
    jugadores.push(pad_2);

    //Pad Rojo
    var pad_3       = new Image();
        pad_3.src   = "img/pad_3.jpg";
    jugadores.push(pad_3);

    //Pad Verde
    var pad_4       = new Image();
        pad_4.src   = "img/pad_4.jpg";
    jugadores.push(pad_4);

//--------------Poderes---------------//
var imgpoderes = [];

    //SuperBola
    var poder_1     = new Image();
        poder_1.src = "img/poderes/superbola.png";
    imgpoderes.push(poder_1);

    //MiniBola
    var poder_2     = new Image();
        poder_2.src = "img/poderes/minibola.png";
    imgpoderes.push(poder_2);

    //AceleraBola
    var poder_3     = new Image();
        poder_3.src = "img/poderes/acelerabola.png";
    imgpoderes.push(poder_3);

    //DeceleraBola
    var poder_4     = new Image();
        poder_4.src = "img/poderes/decelerabola.png";
    imgpoderes.push(poder_4);

    //MultiBola
    var poder_5     = new Image();
        poder_5.src = "img/poderes/multibola.png";
    imgpoderes.push(poder_5);

    //TeleBola
    var poder_6     = new Image();
        poder_6.src = "img/poderes/telebola.png";
    imgpoderes.push(poder_6);

    //SuperJugador
    var poder_7     = new Image();
        poder_7.src = "img/poderes/superjugador.png";
    imgpoderes.push(poder_7);

    //MiniJugador
    var poder_8     = new Image();
        poder_8.src = "img/poderes/minijugador.png";
    imgpoderes.push(poder_8);

    //AceleraJugador
    var poder_9     = new Image();
        poder_9.src = "img/poderes/acelerajugador.png";
    imgpoderes.push(poder_9);

    //DeceleraJugador
    var poder_10     = new Image();
        poder_10.src = "img/poderes/decelerajugador.png";
    imgpoderes.push(poder_10);

    //CongelaJugador
    var poder_11     = new Image();
        poder_11.src = "img/poderes/congelajugador.png";
    imgpoderes.push(poder_11);
